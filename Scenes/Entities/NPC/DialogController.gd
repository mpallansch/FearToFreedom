...
func _process(delta):
	if Input.is_action_just_pressed("interact") && player_close:
		if !dialog_interface.is_visible_in_tree():
			PlayerVariables.vertical_lock = true
	...
	if Input.is_action_just_pressed("up") && player_close && dialog_interface.is_visible_in_tree():
		if highlighted > 0:
			highlighted -= 1
			render_dialog_interface()
	if Input.is_action_just_pressed("down") && player_close && dialog_interface.is_visible_in_tree():
...
func _on_area_2d_body_exited(body):
	if body.owner.name == "Player":
		player_close = false
		dialog_interface.hide()
		PlayerVariables.vertical_lock = false
